...
const Details = () => {
  const { setDetails } = useContext(IntelContext)

  return (
    <Box df fdc s={{ flexGrow: 1, justifyContent: 'center' }}>
      <Spacer size="l">
        <Input huge placeholder="Title" onChange={(value: string) => setDetails((details) => ({ ...details, title: value}))} />
        <Input huge placeholder="Description" onChange={(value: string) => setDetails((details) => ({ ...details, description: value}))} />
      </Spacer>
    </Box>
  )
}
...
const Submit = ({ id }: { id: string }) => {
  ...
  return (
    <Spacer s={{ flexGrow: 1, justifyContent: 'center' }}>
      <Button onClick={submitData}>{ id === 'new' ? 'Initialize' : 'Join' } playlist</Button>
    </Spacer>
  )
}
