...
export function useSpotify(shouldAuthenticate: boolean, clientId: string, redirectUrl: string, scopes: string[], config?: SdkOptions) {
  const [sdk, setSdk] = useState<SpotifyApi | null>(null);
  const { query: { code } } = useRouter()
  const { current: activeScopes } = useRef(scopes);

  useEffect(() => {
    if (shouldAuthenticate || code) {
      (async () => {
  ...
  }, [shouldAuthenticate, clientId, redirectUrl, config, activeScopes]);
